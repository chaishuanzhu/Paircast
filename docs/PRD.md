...
> 变更摘要（v1.5）：配置分享改为加密深链 `tandem://config?args=`（打开链接或粘贴导入；可选二维码承载同一 URL）
...
| 配置分享导入成功率 | 合法 `tandem://config?args=` 链接一次确认导入后可正常登录/列片库 | ≥ 95% |
...
2. 保存后返回登录；可用预置账号登录  
3. 已配置用户进入配置页 → **分享配置** → 得到 `tandem://config?args=…` 加密链接（可复制/系统分享；可选二维码承载同一链接）→ 好友打开链接或粘贴后确认写入同一套环境  
...
- Deep Link：`tandem://watch?...`、`tandem://config?args=…` 进入后落到对应 SPA 路由态  
...
在 App 内配置或通过加密分享链接导入七牛与腾讯云 IM；支持导出 `tandem://config?args=` 供他人打开或粘贴自动配置。
...
| 打开配置深链 | `tandem://config?args=…` → 进配置页并弹出确认导入 |
...
`tandem://config?args=<base64url>` → 可复制、系统分享；可选将同一 URL 渲染为二维码存相册。强提示含密钥风险。
...
```
tandem://config?args=ABCEDbase64url…
```
...
```
登录页 → 服务配置 → 保存 / 粘贴导入 / 打开 tandem://config?args=
...
- [x] 配置加密分享链接（`tandem://config?args=`）/ 打开或粘贴导入  
- [x] 登录（预置账号，无注册）  
- [x] 个人信息（昵称、头像）  
- [x] 电影列表（mp4/mkv，豆瓣优先）  
- [x] 播放页：播放器、成员、邀请、聊天  
...
| 服务配置 | 填写/测试/保存；粘贴导入；分享 `tandem://config?args=` |
