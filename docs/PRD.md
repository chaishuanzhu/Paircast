...
> 变更摘要（v1.5）：配置分享改为加密深链 `paircast://config?args=`（打开链接或粘贴导入；可选二维码承载同一 URL）
...
**Paircast** 是一款 **iOS 原生 App**，让两人（或多人也预留扩展）在同一部影片的播放房间内：

1. 首次配置七牛云与腾讯云 IM（可粘贴/打开配置分享链接导入），再用预置账号登录，维护昵称/头像
...
| 配置分享导入成功率 | 合法 `paircast://config?args=` 链接一次确认导入后可正常登录/列片库 | ≥ 95% |
...
| 片源封装 | 主要 **mp4、mkv** |
| 元数据 | **本地 NFO 优先**，否则使用 TMDB 官方 API，再降级文件名 |
| 云服务 | **七牛云、腾讯云 IM 均可在 App 内配置**；支持加密分享链接导出 / 打开或粘贴导入 |
...
4. **房主**下载外挂/在线字幕后会上传七牛并通知成员加载同一份；**成员**自行下载/选择仅本机生效
5. 字幕时间轴偏移 **仅本机生效**，不影响对方语言偏好
...
房主点选外挂/在线字幕
  → 下载到本机并加载
  → 上传七牛到影片同目录同名（`{movieBase}.srt|vtt|ass`）
  → 播控信令 subtitle_change{ subtitleObjectKey, subtitleLabel }
成员收到信令
  → 预签名 GET 下载 → 本机加载（保留本机偏移）
```

同目录同名外挂也会出现在「片库外挂」列表。内嵌字幕轨无法导出文件，不同步。字幕同步（偏移）始终本机。

**场景 E：首次配置云服务 / 分享给好友**

1. 打开 App → 登录页点「服务配置」→ 填写七牛 + IM，或 **粘贴配置链接导入**  
2. 保存后返回登录；可用预置账号登录  
3. 已配置用户进入配置页 → **分享配置** → 得到 `paircast://config?args=…` 加密链接（可复制/系统分享；可选二维码承载同一链接）→ 好友打开链接或粘贴后确认写入同一套环境  
...
- Deep Link：`paircast://watch?...`、`paircast://config?args=…` 进入后落到对应 SPA 路由态  
...
2. **仅纳入** `.mp4`、`.mkv`  
3. 元数据：**七牛 NFO/海报优先**；否则 **TMDB 官方 API → 文件名**，并写回同目录
4. 匹配失败则降级展示  
...
同前：仅 mp4/mkv；NFO/TMDB 优先；无元数据仍可播。
...
进入播放页
  ...
  → 顶/底工具栏显示；5 秒无操作自动隐藏；点画面再显示
  → 全屏按钮切换沉浸全屏；返回键在全屏下先退出全屏
  → 房主拖动进度条 → seek 信令 → 全员对齐
...
横向 **头像**（IM 资料 + 七牛签名 URL）横滑；外圈 **环形播放进度**：

- 轨道：半透明描边（透明度 **0.3**）
- 进度弧：`positionMs / durationMs`（本机用播放器进度；其他成员用最近同步进度）
- 房主进度色区分（粉）；成员为系统蓝
- 「N 人在看」+ 同步状态文案；监听成员变更与 **host 变更**
...
在 App 内配置或通过加密分享链接导入七牛与腾讯云 IM；支持导出 `paircast://config?args=` 供他人打开或粘贴自动配置。
...
|------|------|
| **登录页「服务配置」** | 必有；未登录可配 |
| **右上角「我的」→ 服务配置** | 已登录可改配/分享 |
| 打开配置深链 | `paircast://config?args=…` → 进配置页并弹出确认导入 |
| 粘贴导入 | 粘贴完整链接或 `args` 密文后确认 |
...
**分享配置：** 先风险确认 → 将配置序列化为 JSON → **AES-GCM 加密** → 生成深链  
`paircast://config?args=<base64url>` → 可复制、系统分享；可选将同一 URL 渲染为二维码存相册。强提示含密钥风险。

**导入配置：**
1. 对方 **打开链接**（Universal / Custom URL）或在配置页 **粘贴链接**  
2. 脱敏预览确认（将覆盖本机配置）→ 写入 Keychain →「配置成功」  
3. 非法/篡改密文不写入、不覆盖  
...
```
paircast://config?args=ABCEDbase64url…
```

| 字段 | 说明 |
|------|------|
| scheme / host | 固定 `tandem` / `config` |
| `args` | AES-GCM sealed box 的 base64url（无 padding）；明文为下方 JSON |

**明文 JSON（加密前，与历史配置码兼容）：**
...
- 明文 UTF-8 ≤ 2KB；`args` 过长则提示失败（后续可短链托管）  
- 加密密钥为 App 内嵌派生，仅作传输混淆；**链接等价于环境根密钥，仅发给可信好友**  
- 导入仍兼容旧版明文 JSON（便于迁移）
...
- [ ] 保存后可登录、可列片库  
- [ ] 测试连接区分 IM/七牛失败  
- [ ] 分享链接 ↔ 另一设备打开/粘贴导入一致可用  
- [ ] 退出登录保留配置；非法链接不污染现有配置  
...
```
登录页 → 服务配置 → 保存 / 粘贴导入 / 打开 paircast://config?args=
片库主壳 → 右上角「我的」 → 改昵称头像 / 服务配置分享加密链接
```
...
**元数据：** 七牛同目录 NFO/海报/背景图优先 → 缓存 → **TMDB 官方 API** → 文件名降级；补全结果写回 `{主名}.nfo`、`{主名}-poster.jpg`、`{主名}-fanart.jpg`
...
|------|------|
| 无公开注册 | 预置账号 |
| 配置密钥 | 存 Keychain；UI 掩码；分享链接含密钥须警告 |
| 链接导入 | 确认页脱敏预览；防误覆写「覆盖确认」 |
...
| 配置链接非法/过长/篡改 | 提示失败；不覆盖现有配置；过长走短链方案（后续） |
| 群主离开 | 自动转让；仅一人则结束房间 |
...
- [x] 配置加密分享链接（`paircast://config?args=`）/ 打开或粘贴导入  
- [x] 登录（预置账号，无注册）  
- [x] 个人信息（昵称、头像）  
- [x] 电影列表（mp4/mkv，NFO/TMDB 优先）
- [x] 播放页：播放器、成员、邀请、聊天  
...
4. **配置页**：测试连接反馈分区；分享前二次风险确认。  
5. **配置导入**：仅接受合法加密 `args`（或兼容明文 `type=tandem-config`）；确认后写入。  
...
| 8 | 群主离开 | **自动转让** |
| 9 | 云服务 | 七牛+IM **可配置**；配置页；加密分享链接；登录页入口 |
...
| 服务配置 | 填写/测试/保存；粘贴导入；分享 `paircast://config?args=` |
