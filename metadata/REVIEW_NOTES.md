...
3. Log in with the IM user ids below. The password field only needs any non-empty value for this demo (IM login uses UserSig).
...
- User A (host): `REVIEW_USER_A` / any non-empty password
- User B (guest): `REVIEW_USER_B` / any non-empty password
